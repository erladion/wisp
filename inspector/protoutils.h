...
#include "broker.pb.h"
#include "anyframe.h"  // the Any type url prefix the client library writes
...
inline void drawEnvelopeAndPayload(const broker::MessageHeader& header, const std::string& payload, QTreeWidget* treeWidget) {
  ...
  if (any.ParseFromString(payload) && any.type_url().rfind(std::string(Wisp::AnyFrame::TYPE_URL_PREFIX), 0) == 0) {
